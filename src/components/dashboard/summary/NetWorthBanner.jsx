...
export function NetWorthBanner() {
  ...
  const stats = useMemo(() => {
    ...
    // SIMPLE MODE: Only sum SIP & Savings. PRO MODE: Sum everything.
    const totalInvested = isProMode
        ? sumSeries(sipSeries) + sumSeries(savSeries) + sumSeries(epfSeries) + sumSeries(vpfSeries)
        : sumSeries(sipSeries) + sumSeries(savSeries);
    
    // SIMPLE MODE: Only take final Corpus of SIP & Savings. PRO MODE: Take full Net Worth.
    let totalValue = 0;
    if (isProMode) {
        totalValue = summary.total || 0;
    } else {
        const sipFinal = sipSeries?.[sipSeries.length - 1]?.corpusNominal || 0;
        const savFinal = savSeries?.[savSeries.length - 1]?.corpusNominal || 0;
        totalValue = sipFinal + savFinal;
    }

    const totalGains = totalValue - totalInvested;

    return { totalValue, totalInvested, totalGains };
  }, [summary, sipSeries, savSeries, epfSeries, vpfSeries, isProMode]);

  return (
    <div className="w-full bg-gradient-to-br from-slate-800 to-slate-900 dark:from-slate-900 dark:to-black z-0 text-white p-8 md:p-10 rounded-[32px] text-center shadow-2xl relative overflow-hidden group">
      {/* Decorative Glow */}
      ...
      <div className="relative z-10">
        <h2 className="text-lg md:text-xl font-medium text-slate-400 mb-2">
            {isProMode ? "Estimated Total Net Worth" : "You're On Track To Accumulate"}
        </h2>
